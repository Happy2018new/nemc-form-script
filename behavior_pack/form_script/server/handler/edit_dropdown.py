...
class EditDropdownHandler:
    """
    EditDropdownHandler 是所有 /editdropdown 命令的处理设备
    """
    ...
    def __init__(self, storage, feature):  # type: (FormStorage, FormFeature) -> None
        """初始化并返回一个新的 EditDropdownHandler
    ...
    def on_custom_command_trigger(self, args):  # type: (dict[str, Any]) -> None
        ...
        有必要确保它的上层调用者验证了调用的命令是 /editdropdown
    ...
    def handle_append(self, args):  # type: (list[dict[str, Any]]) -> str
        ...
        option_code = args[3]["value"]  # type: str
        ...
        with self.storage.get_locker():
            ...
            real_option_code = StringWithHash(option_code)
            _ = self.feature.executor.compile_cache.get_runner(real_option_code)
            element.options.append(real_option_code)

            return "commands.editdropdownappend.success"
    ...
    def handle_default(self, args):  # type: (list[dict[str, Any]]) -> str
        ...
        with self.storage.get_locker():
            ...
            return "commands.editdropdowndefault.success"
    ...
    def handle_insert(self, args):  # type: (list[dict[str, Any]]) -> str
        ...
        option_code = args[4]["value"]  # type: str
        ...
        with self.storage.get_locker():
            ...
            if not isinstance(element, ModalStorageFormElementDropdown):
                raise Exception("commands.editdropdown.elementnotmatch")
            if sub_index < 0 or sub_index > len(element.options):
                raise Exception(
                    "给出的子索引 {} 超出长度 {}".format(
                        sub_index, len(element.options)
                    )
                )

            real_option_code = StringWithHash(option_code)
            _ = self.feature.executor.compile_cache.get_runner(real_option_code)
            element.options.insert(sub_index, real_option_code)

            return "commands.editdropdowninsert.success"
    ...
    def handle_pop(self, args):  # type: (list[dict[str, Any]]) -> str
        ...
        with self.storage.get_locker():
            ...
            if len(element.options) == 0:
                raise Exception("commands.editdropdownpop.failed")
            if pop_action == POP_RAW_ACTION_LEFT:
                _ = element.options.pop(0)
            elif pop_action == POP_RAW_ACTION_RIGHT:
                _ = element.options.pop(-1)

            return "commands.editdropdownpop.success"
    ...
    def handle_sub(self, args):  # type: (list[dict[str, Any]]) -> str
        ...
        with self.storage.get_locker():
            ...
            if start_index < 0 or start_index > len(element.options):
                raise Exception(
                    "给出的起始索引 {} 超出长度 {}".format(
                        start_index, len(element.options)
                    )
                )
            if end_index < 0 or end_index > len(element.options):
                raise Exception(
                    "给出的结束索引 {} 超出长度 {}".format(
                        end_index, len(element.options)
                    )
                )
            ...
            if sub_action == SUB_RAW_ACTION_KEEP:
                element.options = element.options[start_index:end_index]
                return "commands.editdropdownsubkeep.success"
            elif sub_action == SUB_RAW_ACTION_DISCARD:
                element.options = (
                    element.options[:start_index] + element.options[end_index:]
                )
                return "commands.editdropdownsubdiscard.success"
    ...
    def handle_text(self, args):  # type: (list[dict[str, Any]]) -> str
        ...
        with self.storage.get_locker():
            ...
            return "commands.editdropdowntext.success"
    ...
    def handle_tooltip(self, args):  # type: (list[dict[str, Any]]) -> str
        ...
        with self.storage.get_locker():
            ...
            return "commands.editdropdowntooltip.success"
