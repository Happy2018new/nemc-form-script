...
def _generate_long_form(
    ...
    title = runner.run_code(form.title, executor, dimension, position, True)
    ...
    content = runner.run_code(form.content, executor, dimension, position, True)
    ...
    for index, value in enumerate(form.buttons):
        ...
        # text
        text = runner.run_code(value.text, executor, dimension, position, True)
...
def _generate_popup_form(
    ...
    button1 = runner.run_code(form.button1, executor, dimension, position, True)
    ...
    button2 = runner.run_code(form.button2, executor, dimension, position, True)
    ...
    for index, value in enumerate(form.content):
        if isinstance(value, ModalStorageFormElementLabel):
            ...
            for ind, val in enumerate(value.options):
                option = runner.run_code(val, executor, dimension, position, True)
            ...
            # step
            step = runner.run_code(value.step, executor, dimension, position, True)
            ...
            for ind, val in enumerate(value.steps):
                step = runner.run_code(val, executor, dimension, position, True)
