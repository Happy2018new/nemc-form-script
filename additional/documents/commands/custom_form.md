...
## 备注二
如果表单的关闭是因为玩家在提交表单前退出了游戏，则您应确保 `<code: string>` 和 `[onCodeError: string]` 中的代码**不会**向玩家打开新的表单。<br/>
系统不会阻止您在这种情况下打开新的表单，但您**永远不应该**这么做。这是因为，这么做将会导致新打开的表单永远不会得到回应，从而造成内存泄露。
